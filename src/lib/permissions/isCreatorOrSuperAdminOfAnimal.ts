...
export const isCreatorOrSuperAdminOfAnimal = async (
  animalId: number,
  ...
  if (
    membership?.status !== MemberStatus.VALIDATED ||
    (membership?.role !== MemberRole.SUPERADMIN && user.id !== animalPrev.createdByMemberId)
  ) {
    return {
